...
class Server
{
    ...
    public function run()
    {
        ...
        $this->params['sign_method'] = !empty($this->params['sign_method']) ? $this->params['sign_method'] : $this->sign_method;
        ...
        if (count($classNameList) == 1) {
            ...
            $return[$return_list['data_name']] = $return_list['data'] ?? [];
        } else {
        ...
        return ['status' => 1, 'code' => '200', 'current_time' => date('Y-m-d H:i:s'), 'data' => $return];
    }
    ...
    public function executeClassFunction(array $method_list)
    {
        ...
        $this->params['function_name'] = (count($method_list) > 1) ? $method_list[1] : '';//执行类中的某个方法----默认执行run

        // D.2 判断类名是否存在---配置API控制器文件夹路径
        ...
        $classPath = (empty($api_controller_location) ? __NAMESPACE__ . '\\Response\\' : $api_controller_location . '\\') . $className;
        if (!$className || !class_exists($classPath)) return ['data_name' => $className, 'data' => ['status' => 0, 'code' => '404', 'msg' => '类 {' . $className . '} 不存在！']];
        // E. api接口分发
        $class = new $classPath;

        return ['data' => (array)$class::apiReturn($class, $this->params, $className), 'data_name' => $className];
    }
